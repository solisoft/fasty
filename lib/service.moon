...
install_service = (sub_domain, name)->
  ...
  -- Install the service
  os.execute("cd install_service/#{sub_domain}/#{name}/APP && npm i")
...
deploy_site = (sub_domain, settings) ->
  ...
  os.execute("arangodump --collection layouts --collection partials --collection components --collection spas --collection redirections --collection datatypes --collection aqls --collection helpers --collection apis --collection api_libs --collection api_routes --collection api_scripts --collection api_tests --collection sripts --collection pages --collection trads --collection uploads --collection folder_path --collection folders --collection datasets --include-system-collections true --server.database db_#{sub_domain} --server.username #{db_config.login} --server.password #{db_config.pass} --server.endpoint http+tcp://172.31.0.6:8529 --output-directory #{path} --overwrite true")
